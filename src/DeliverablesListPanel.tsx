...
const PAGE_SIZE = 10;
/** Match other screens: fetch with `top` + `orderBy` only; paginate in memory (Dataverse `skip` is unreliable in this host). */
const FETCH_TOP = 5000;

function statusBadgeClass(statusLabel: string): string {
  const u = statusLabel.toUpperCase();
  if (u.includes('DELIVERED') && !u.includes('TO') && !u.includes('BE')) {
    return 'enj-table-status--completed';
  }
...
export function DeliverablesListPanel({ isActive, refreshKey, onNotify, variant, onNewDeliverable }: Props) {
  ...
  const loadAll = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await New_deliverablesService.getAll({
        top: FETCH_TOP,
  ...
  const statusCounts = (() => {
    ...
    chartSample.forEach((r) => {
      const code = r.new_status;
      if (code === 100000000) c.delivered += 1;
      else if (code === 100000001) c.toDeliver += 1;
      else if (code === 100000002) c.delayed += 1;
    });
    return c;
  })();
  ...
  const btnNew = `${enj.btn} ${enj.btnPrimary} px-4 ${
    variant === 'program' ? 'text-sm font-medium' : 'text-xs font-semibold'
  }`;
    ...
    <div className="relative bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <table className={`${enj.table} w-full text-xs`}>
        ...
        <tbody>
          {loading ? (
            <tr>
              <td colSpan={4} className="px-3 py-6 text-sm text-gray-500 text-center">
                Loading…
              </td>
            </tr>
          ) : rows.length === 0 ? (
            <tr>
              <td colSpan={4} className="px-3 py-6 text-sm text-center">
                {loadError ? (
            ...
            rows.map((row, rowIdx) => {
              ...
              const rowKey = String(
                (row as { new_deliverableid?: string }).new_deliverableid ??
                  (row as { new_deliverablesid?: string }).new_deliverablesid ??
              ...
              return (
                <tr key={rowKey} className="text-xs">
                  ...
                  <td className="px-3 py-2 max-w-[280px] truncate" title={row.new_thedeliverablesinclude ?? row.new_notes ?? ''}>
                    {String(row.new_thedeliverablesinclude ?? row.new_notes ?? '—')}
                  </td>
                  <td className="px-3 py-2">
                    <span className={`enj-table-status ${statusBadgeClass(st)} text-[9px]`}>{st}</span>
                  </td>
                </tr>
              );
            })
    ...
    <aside className="space-y-3">
      <div className="bg-white rounded-xl p-3 shadow-sm chart-card">
        ...
        <p className="text-[9px] text-gray-500 mb-2">
          Sample: last {chartSample.length} records — Delivered: {statusCounts.delivered}, To deliver: {statusCounts.toDeliver}, Delayed:{' '}
